...
def show_map(disasters_data, alerts_data):
    ...
    if alerts_data:
        ...
        if "location" in df_alerts.columns:
            ...
            df_alerts["color"] = df_alerts["severity"].apply(lambda x: color_map.get(x,[0,0,255,160]))
            map_layers.append(
                pdk.Layer(
                    "ScatterplotLayer",
                    data=df_alerts,
    ...
    if map_layers:
        ...
        st.pydeck_chart(
            pdk.Deck(
                ...
                layers=map_layers,
                tooltip={"html": "<b>Layer:</b> {layer}<br><b>Title:</b> {title}<br>Type: {disaster_type}<br>Severity: {severity}<br>Published: {publishedAt}<br>Source: {source}<br><a href='{url}' target='_blank'>Link</a>"}
            )
        )
    else:
